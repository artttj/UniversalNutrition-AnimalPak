...
class Product extends \Magento\Rule\Model\Condition\AbstractCondition
{
    ...
    protected function _addSpecialAttributes(array &$attributes)
    {
        ...
        $attributes['stock_status'] = __('Stock Status');
    }
    ...
    protected function _prepareValueOptions()
    {
        ...
        if ($this->getAttribute() === 'stock_status') {
            $selectOptions = [
    ...
    public function collectValidatedAttributes($productCollection)
    {
        ...
        if (!in_array($attribute, ['category_ids', 'qty', 'price_diff', 'percent_discount', 'set_as_new', 'stock_status'])) {
            ...
            // $productCollection->addStockStatus();
        }
    ...
    public function validate(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ('category_ids' == $attrCode) {
            ...
            $stockItemResource->loadByProductId(
                $stockItem,
                $object->getId(),
                $this->storeManager->getStore($object->getStoreId())->getWebsite()->getId()
            );
            if ($stockItem->getTypeId() == 'configurable') {
                ...
                if (self::$multiSourceInventorySupported) {
                    ...
                    $stockData = $this->sourceDataBySku->execute($object->getSku());
                        ...
                        if ($source->isEnabled()) {
                            foreach ($stockData as $stockItem) {
                                if ($stockItem['source_code'] == $source->getSourceCode()) {
                                    return $this->validateAttribute($stockItem['quantity']);
                                }
                            }
                        }
                    }
            ...
            if ($object->getTypeId() == \Magento\Bundle\Model\Product\Type::TYPE_CODE) {
                ...
                if (empty($child)) {
                    ...
                    foreach ($child as $option) {
                        ...
                        $stockItem = $option->getExtensionAttributes()->getStockItem();
            ...
            } elseif (!$object->isAvailable() || !$object->getExtensionAttributes()->getStockItem()) {
                $return = false;
            }
            ...
            if ($final <= 0) {
                $rules = $this->rule
                    ->getRulesFromProduct(
                        $this->dateTime->scopeDate($object->getStoreId())->format(\Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT),
